...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!dataInCache) throw new UnauthorizedException('Access Token is Invalid');

    try {
      this.jwtService.verify(token, { secret: process.env.JWT_SECRET });
    } catch (error) {
      if (error.name === 'TokenExpiredError') {
        const newAccessToken = await this.generateNewAccessToken(token, dataInCache);
  ...
  private async generateNewAccessToken(oldToken: string, dataInCache: any): Promise<string> {
    const { refreshToken, ...payload } = dataInCache;
    this.jwtService.verify(refreshToken, { secret: process.env.JWT_SECRET });

    const newAccessToken = this.jwtService.sign(payload, { secret: process.env.JWT_SECRET });
